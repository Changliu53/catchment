...
interface Props {
  ...
  missing: number;
  /**
   * Set for a comparison. The map is then painting two groups rather than a
   * graded field, and the legend has to name them — otherwise two blues on
   * screen mean nothing.
   */
  split: { field: string; threshold: number; above: number; below: number } | null;
}
