...
const STEP_SCHEMA = {
  ...
  properties: {
    op: {
      type: 'string',
      enum: [...PRIMITIVE_OPS],
      description: 'Which operation this step performs.',
    },
    field: { ...fieldEnum, description: 'filter: the field to compare.' },
    comparison: {
      type: 'string',
      enum: [...COMPARISON_OPS],
      description: 'filter: the comparison.',
    },
    ...
    min_pct: {
      ...
      minimum: 0,
      maximum: 1,
      description: 'flood_exposure: minimum share of area, 0-1.',
    },
    poi_type: {
      type: 'string',
      enum: [...POI_TYPES],
      description: 'resource_gap: which kind of place.',
    },
    max_distance_m: {
      type: 'number',
  ...
  input_schema: {
    ...
    properties: {
      ...
      suggestion: {
        type: 'string',
        maxLength: 300,
        description: 'A related question that does work.',
      },
    },
